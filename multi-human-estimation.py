    ...
    while True:
        ...
        for (xmin, ymin, xmax, ymax, confidence, clas) in result.xyxy[0].tolist():
            with mp_pose.Pose(min_detection_confidence=0.3, min_tracking_confidence=0.3) as pose:
                ...
                cropped_depth_image = depth_image[int(ymin) + MARGIN:int(ymax) + MARGIN, int(xmin) + MARGIN:int(xmax) + MARGIN]
                ...
                for i in range(num):
                    use_lines.extend(
                        [[i + 0 * num + n_p, i + 1 * num + n_p], [i + 1 * num + n_p, i + 7 * num + n_p], [i + 7 * num + n_p, i + 6 * num + n_p],
                         [i + 6 * num + n_p, i + 0 * num + n_p], [i + 1 * num + n_p, i + 3 * num + n_p], [i + 3 * num + n_p, i + 5 * num + n_p],
                         [i + 0 * num + n_p, i + 2 * num + n_p], [i + 2 * num + n_p, i + 4 * num + n_p], [i + 6 * num + n_p, i + 8 * num + n_p],
                         [i + 8 * num + n_p, i + 10 * num + n_p], [i + 7 * num + n_p, i + 9 * num + n_p], [i + 9 * num + n_p, i + 11 * num + n_p]])
