...
class PoseDetector():
    def __init__(self, scene_path, cfg, mode=False, upBody=False, smooth=True, detectionCon=0.5, trackCon=0.5):
        ...
        # # need to use spawn
        # try:
        #     mp.set_start_method('spawn', force=True) #spawn
        # except RuntimeError:
        #     pass

        self.show_points = torch.zeros((120*num,2),dtype=torch.float32)
        self.show_points.share_memory_()
        self.show_depth = torch.zeros(120*num,dtype=torch.float32)
        self.show_depth.share_memory_()
        self.num_people = torch.zeros((1)).int()
        self.num_people.share_memory_()
    ...
    def findPosition(self, img, depth, p, draw=True):
        ...
        while True:
            ...
            for (xmin, ymin, xmax, ymax, confidence, clas) in result.xyxy[0].tolist():
                with mp_pose.Pose(min_detection_confidence=0.3, min_tracking_confidence=0.3) as pose:
                    ...
                    person_points, person_depth = self.findPosition(cropped_image, cropped_depth_image)
            ...
            self.num_people = torch.tensor(num_people).int()
            self.show_points[:12*num_people*num,:] = torch.tensor(show_points, dtype=torch.float32)
            self.show_depth[:12*num_people*num] = torch.tensor(show_depth, dtype=torch.float32)
    ...
    def visualize(self, rank):
        ...
        while True:
            ...
            if self.cfg['render_mode'] == 'centers':
                ...
                pts, cols, pose_lineset, pose_colors, pose_points = ycy_rgbd2pcd(im, depth, w2c, k, self.cfg, t_show_points,
                                                                                 t_show_depth, k_c2w, pose_links)
    ...
    #     for rank in range(2):
    #         if rank == 0:
    #             p = mp.Process(target=self.ycy, args=(rank, ))
    #         elif rank == 1:
    #             p = mp.Process(target=self.ycy, args=(rank, ))
    #         p.start()
    #         processes.append(p)
