...
from ycy_utils.slam_external import is_keyframe, get_visibility
...
'''
def ycy_keyframe_selection_overlap(gt_depth, w2c, intrinsics, keyframe_list, k, pixels=1600):
    """
    Select overlapping keyframes to the current camera observation.
    ...
    list_keyframe = []
    cur_frame_visibility_filter, occ_aware_visibility = get_visibility(params, selected_keyframes, keyframe_list, time_idx)
    for keyframeid, keyframe in enumerate(keyframe_list):
        # Get the estimated world2cam of the keyframe
        est_w2c = keyframe['est_w2c']
        # Transform the 3D pointcloud to the keyframe's camera space
        ...
        ratio = is_keyframe(cur_frame_visibility_filter, occ_aware_visibility[], config):
        list_keyframe.append(
            {'id': keyframeid, 'percent_inside': percent_inside/ratio})
