...
from utils.slam_external import calc_ssim, build_rotation, prune_gaussians, ycy_prune_gaussians, densify, is_keyframe, \
    get_visibility
...
def initialize_first_timestep(dataset, num_frames, scene_radius_depth_ratio, mean_sq_dist_method, densify_dataset=None):
    ...
    for idx in range(num_frames):
        n_color, n_depth, n_intrinsics, n_pose = dataset[idx]

        # Process RGB-D Data
        n_color = n_color.permute(2, 0, 1) / 255  # (H, W, C) -> (C, H, W)
        n_depth = n_depth.permute(2, 0, 1)  # (H, W, C) -> (C, H, W)

        # Process Camera Parameters
        n_intrinsics = n_intrinsics[:3, :3]
        n_w2c = torch.linalg.inv(n_pose)

        # Setup Camera
        n_cam = setup_camera(n_color.shape[2], n_color.shape[1], n_intrinsics.cpu().numpy(), n_w2c.detach().cpu().numpy())

        cam_list.append(n_cam)

    if densify_dataset is not None:
        return params, variables, intrinsics, w2c, cam, densify_intrinsics, densify_cam, cam_list
    else:
        return params, variables, intrinsics, w2c, cam, cam_list
'''
...
def get_loss(params, curr_data, variables, iter_time_idx, loss_weights, use_sil_for_loss,
    ...
    # check = np.count_nonzero(n_touched_rgb.cpu())
    # xiao = n_touched_rgb.cpu().min()
    # print("n_touched_rgb", check)
    # print("xiao", xiao)
    # im, radius, radii, geomBuffer, binningBuffer, imgBuffer, depth, n_touched = Renderer(raster_settings=curr_data['cam'])(**rendervar)
    variables['means2D'] = rendervar['means2D']  # Gradient only accum from colour render for densification

    # Depth & Silhouette Rendering
    depth_sil, _, _, _, _, = Renderer(raster_settings=curr_data['cam'])(**depth_sil_rendervar)
    # print("n_touched_depth", n_touched_depth)
...
def rgbd_slam(config: dict):
    ...
    for time_idx in tqdm(range(checkpoint_time_idx, num_frames)):
        ...
        # gt_cam = setup_camera(color.shape[2], color.shape[1], intrinsics.cpu().numpy(), gt_w2c.detach().cpu().numpy())
        # Optimize only current time step for tracking
        iter_time_idx = time_idx
        if time_idx == 0:
            curr_w2c = first_frame_w2c
        curr_cam = setup_camera(color.shape[2], color.shape[1], intrinsics.cpu().numpy(),
                                curr_w2c.detach().cpu().numpy())
        # Initialize Mapping Data for selected frame
        curr_data = {'cam': cam, 'im': color, 'depth': depth, 'id': iter_time_idx, 'intrinsics': intrinsics,
                     'w2c': first_frame_w2c, 'iter_gt_w2c_list': curr_gt_w2c}  # 'cam': cam 'w2c': first_frame_w2c
        ...
        if seperate_tracking_res:
            ...
            tracking_curr_data = {'cam': tracking_cam, 'im': tracking_color, 'depth': tracking_depth,
                                  'id': iter_time_idx,
                                  'intrinsics': tracking_intrinsics, 'w2c': first_frame_w2c,
        ...
        if time_idx > 0 and not config['tracking']['use_gt_poses']:
            ...
            while True:
                ...
                if iter == num_iters_tracking:
                    if losses['depth'] < config['tracking']['depth_loss_thres'] and config['tracking'][
        ...
        if time_idx == 0 or (time_idx + 1) % config['map_every'] == 0:
            ...
            if config['mapping']['add_new_gaussians'] and time_idx > 0:
                ...
                if seperate_densification_res:
                    ...
                    densify_curr_data = {'cam': densify_cam, 'im': densify_color, 'depth': densify_depth,
                                         'id': time_idx,
            ...
            for occ_idx in range((len(selected_time_idx))):
                kf_idx = selected_time_idx[occ_idx]

                _, _, _, n_pose = dataset[kf_idx]
                transformed_pts = transform_to_frame(params, kf_idx, gaussians_grad=False, camera_grad=False)
                # Process poses
                n_w2c = torch.linalg.inv(n_pose)
                n_cam = setup_camera(color.shape[2], color.shape[1], intrinsics.cpu().numpy(),
                                     n_w2c.detach().cpu().numpy())
                rgb_rendervar = transformed_params2rendervar(params, transformed_pts)
                _, _, _, n_touched_rgb, = Renderer(raster_settings=n_cam)(**rgb_rendervar)
            ...
            for iter in range(num_iters_mapping):
                ...
                iter_data = {'cam': cam, 'im': iter_color, 'depth': iter_depth, 'id': iter_time_idx,
                             ...
                             'iter_gt_w2c_list': iter_gt_w2c}  # 'cam': cam 'w2c': first_frame_w2c
                '''
                ## ycy edit
                n_touched_acm = []

                for n_idx in range(len(selected_time_idx)):
                    transformed_pts = transform_to_frame(params, selected_time_idx[n_idx], gaussians_grad=False, camera_grad=False)

                    _, _, _, n_pose = dataset[selected_time_idx[n_idx]]
                    # Process poses
                    n_w2c = torch.linalg.inv(gt_pose)
                    n_cam = setup_camera(color.shape[2], color.shape[1], intrinsics.cpu().numpy(),
                                          n_w2c.detach().cpu().numpy())
                    ...
                    #depth_rendervar = transformed_params2depthplussilhouette(params, curr_data['w2c'], transformed_pts)
                    #_, _, _, n_touched_depth, = Renderer(raster_settings=cam_list[selected_time_idx[cam_idx]])(**depth_rendervar)

                    n_touched_acm.append(n_touched_rgb)
                '''
                # Loss for current frame
                ...
                with torch.no_grad():
                    ...
                    if config['mapping']['prune_gaussians']:
                        # print("params", params)
                        params, variables = prune_gaussians(params, variables, optimizer, iter + 1,
                                                            config['mapping']['pruning_dict'])
                        # params, variables = ycy_prune_gaussians(params, variables, optimizer, iter, config['mapping']['pruning_dict'], selected_keyframes, config['mapping_window_size'], keyframe_list, iter_data, time_idx)
                        if config['use_wandb']:
                            wandb_run.log({"Mapping/Number of Gaussians - Pruning": params['means3D'].shape[0],
                                           "Mapping/step": wandb_mapping_step})
                    # Gaussian-Splatting's Gradient-based Densification
                    if config['mapping']['use_gaussian_splatting_densification']:
                        params, variables = densify(params, variables, optimizer, iter,
                                                    config['mapping']['densify_dict'], time_idx)
                        if config['use_wandb']:
        ...
        not torch.isnan(curr_gt_w2c[-1]).any()):
            with torch.no_grad():
                ...
                if time_idx > config['keyframe_every']:
                    cur_frame_visibility_filter, occ_aware_visibility, out_opacity = get_visibility(params, selected_keyframes, keyframe_list, curr_data, time_idx, curr_w2c)
                    flag = is_keyframe(curr_w2c, keyframe_list[-1]['est_w2c'], cur_frame_visibility_filter, occ_aware_visibility, config, depth, out_opacity)
                    print(f"\nycy_flag:{flag}")
                '''
                if flag:
                    # Initialize Keyframe Info
